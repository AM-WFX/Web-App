...
NT         targetSelector: ".card-header > h2",
...
s         targetSelector: "h3 ~ button.comments",
...
  F         <input type="text" name="data-pass" placeholder="Password">
        `
    },
    {
        id: 8,
...
Rules         else if (challengeDef.type.includes('General')) operatorHint = 'Use the <b>~</b> operator.';
...
    const inputField = document.getElementById(`selector-input-${challengeId}`);
    const challengeDef = challengeDefinitions.find(d => d.id === challengeId);
EOT     const statusElement = document.getElementById(`status-${challengeId}`);
...
N         <button class="accordion" style="background-color: #007bff; color: white; margin-right: 5px;" onclick="resetChallenge(${challengeId})">🔄 Try New ${def.type} Challenge</button>
  t   `;
    // Highlighting removal no longer needed
}
...
s     statusElement.textContent = "(Unsolved)";
